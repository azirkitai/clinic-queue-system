...
- **UI/UX**: Shadcn/ui (Radix UI) and Tailwind CSS with Material Design principles, custom theme system with light/dark modes.
  - **Visual Theme**: Default is **Dark Gradient Mode** (`defaultTheme="dark"` in `App.tsx`). Body uses a multi-layer gradient: deep navy → teal → cyan glow with two soft radial highlights (`linear-gradient(160deg, hsl(215 40% 6%) → hsl(195 50% 12%))` plus radial accents at top-right & bottom-left). `--background: 215 35% 8%`, `--card: 210 35% 13%`, `--primary: 188 85% 50%` (cyan). Sidebar uses a premium **indigo→violet gradient** (`#1e1b4b → #4c1d95 → #7c3aed`) defined via `.bg-sidebar`/`.sidebar-gradient`. The `.btn-gradient` utility (135deg violet→purple, `#4c1d95 → #7c3aed → #a855f7`) styles primary action buttons (Fullscreen TV, Login). Light mode preserved (sky-blue primary, white cards) for users who toggle. Stat numbers use cohesive Tailwind `-600` semantic colors: amber (Waiting), sky (Called), violet (Dispensary), emerald (Completed), primary (Active Rooms).
